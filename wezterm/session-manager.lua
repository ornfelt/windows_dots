...
local function retrieve_workspace_data(window)
  ...
  -- Skip saving if there are fewer than 3 tabs
  local tabs = window:mux_window():tabs()
  if #tabs < 3 and os == "x86_64-pc-windows-msvc" then
    return nil
  end

  -- Skip saving unless three unique cwds
  local unique_cwds = {}
  local cwd_map = {}

  for _, tab in ipairs(window:mux_window():tabs()) do
    for _, pane_info in ipairs(tab:panes_with_info()) do
      local cwd = decode_url(tostring(pane_info.pane:get_current_working_dir()))
  ...
  if #unique_cwds < 3 and os == "x86_64-pc-windows-msvc" then
    return nil
  end
...
function session_manager.restore_state(window)
  ...
  --local file_path = wezterm.home_dir .. "/.config/wezterm/wezterm-session-manager/wezterm_state_" .. workspace_name .. ".json"
  local file_path = wezterm.home_dir .. "/.wezterm/wezterm-session-manager/wezterm_state_" .. workspace_name .. ".json"

  if os == "x86_64-unknown-linux-gnu" then
    file_path = wezterm.home_dir .. "/.config/wezterm/wezterm-session-manager/wezterm_state_" .. workspace_name .. ".json"
  end
  ...
  if recreate_workspace(window, workspace_data) then
    status.notify(window, 'WezTerm', 'Workspace state loaded for workspace: ' .. workspace_name, true)
  else
    status.notify(window, 'WezTerm', 'Workspace state loading failed for workspace: ' .. workspace_name, false)
  end
end
...
function session_manager.save_state(window)
  ...
  -- Skip saving if data is nil
  if data == nil then
    status.notify(window, 'WezTerm Session Manager', 'Less than 3 tabs; skipping save.', false)
    return
  end

  -- Construct file path based on workspace name
  local file_path = wezterm.home_dir .. "/.wezterm/wezterm-session-manager/wezterm_state_" .. data.name .. ".json"
  if os == "x86_64-unknown-linux-gnu" then
    file_path = wezterm.home_dir .. "/.config/wezterm/wezterm-session-manager/wezterm_state_" .. data.name .. ".json"
  end
  ...
  if save_to_json_file(data, file_path) then
    ...
    status.notify(window, 'WezTerm Session Manager', 'Failed to save workspace state. file_path: ' .. file_path, false)
  end
end
